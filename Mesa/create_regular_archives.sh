...
create_regular_archive() {
    ...
    # Get the relative path from build-switch
    local rel_path=$(echo "$archive_path" | sed 's|build-switch/||')
    local lib_dir=$(dirname "$rel_path")
    
    # Find object files that might belong to this library
    if [ -d "build-switch/$lib_dir" ]; then
        # Look for .o files in the same directory
        while IFS= read -r obj_file; do
            obj_files+=("$obj_file")
        done < <(find "build-switch/$lib_dir" -name "*.o" 2>/dev/null)
    fi
    ...
    if [ ${#obj_files[@]} -eq 0 ]; then
        ...
        while IFS= read -r obj_file; do
            obj_files+=("$obj_file")
        done < <(find build-switch -name "*.o" -path "*$lib_base*" 2>/dev/null)
    fi
