...
if [ ! -d "build-complete" ]; then
    echo "❌ Error: build-complete directory not found. Run build_real_mesa.sh first."
    exit 1
fi
...
create_regular_archive() {
    ...
    # Get the relative path from build-complete
    local rel_path=$(echo "$archive_path" | sed 's|build-complete/||')
    local lib_dir=$(dirname "$rel_path")
    
    # Find object files that might belong to this library
    if [ -d "build-complete/$lib_dir" ]; then
        # Look for .o files in the same directory
        while IFS= read -r obj_file; do
            obj_files+=("$obj_file")
        done < <(find "build-complete/$lib_dir" -name "*.o" 2>/dev/null)
    fi
    ...
    if [ ${#obj_files[@]} -eq 0 ]; then
        ...
        while IFS= read -r obj_file; do
            obj_files+=("$obj_file")
        done < <(find build-complete -name "*.o" -path "*$lib_base*" 2>/dev/null)
    fi
...
libraries=(
    "build-complete/src/mapi/es2api/libGLESv2.a"
    "build-complete/src/mapi/glapi/libglapi_static.a"
    "build-complete/src/util/libmesa_util.a"
    "build-complete/src/gallium/drivers/softpipe/libsoftpipe.a"
    "build-complete/src/util/blake3/libblake3.a"
    "build-complete/src/mesa/libmesa.a"
)
...
find build-complete -name "*.a" -exec file {} \; | sort
